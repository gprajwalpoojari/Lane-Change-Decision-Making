...
class Environment:
    ...
    def get_reward(self, act, decoded_data, grid, lane):
        if act == 0:
            reward = (2 * ((decoded_data[3] - 25.0)) / 5)# - abs(ego_car_lane - 1))
        else:
            reward = (2 * ((decoded_data[3] - 25.0)) / 5) - 3
    ...
    def run(self):
        ...
        while self.episode <= self.episodes:    
            ...
            flag = False
            agent.reinitialize()
            episode_fraction = self.episode / self.episodes
            if (episode_fraction > 0.9):
                self.epsilon = 0
            elif (episode_fraction > 0.5):
                self.epsilon = 0.5
            elif (episode_fraction > 0.25):
                self.epsilon = 0.9
            while True:
                try:
                    data = conn.recv(2000)
                ...
                action = agent.actor.act([state, pos], self.epsilon)                
